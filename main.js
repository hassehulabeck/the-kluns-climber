...
function startGame() {
  ...
  // Buttons to click instead of keys - if user is on mobile.
  let weapons = document.getElementById("weapon-buttons");
  weapons.addEventListener("click", listen);
}
...
function listen(e) {
  ...
  if (e.key == "s" || e.key == "S" || e.target.id == "rock") {
    Hero.hero.weapon = 0;
    fight();
  }
  if (e.key == "x" || e.key == "X" || e.target.id == "scissors") {
    Hero.hero.weapon = 1;
    fight();
  }
  if (e.key == "p" || e.key == "P" || e.target.id == "paper") {
    Hero.hero.weapon = 2;
    fight();
  }
...
function checkKluns(foeId) {
  ...
  // Vinner Foe?
    ...
    Hero.hero.weapon == Foe.foes[foeId].weapon + 1 ||
  ...
  if (Hero.hero.health <= 0) gameOver = true;
}
